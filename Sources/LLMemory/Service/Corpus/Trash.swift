...
public struct Trash: Sendable {
    ...
    public func pathFor(_ relativePath: String) throws -> URL {
        layout.trash.appendingPathComponent(try relativeToNotes(relativePath))
    }
    ...
    public func file(_ source: URL, reason: String, now: Int) throws -> URL? {
        ...
        let relativePath = try layout.requireRelative(of: source)
    ...
    public func destination(of source: URL) -> URL? {
        guard let relativePath = try? layout.requireRelative(of: source) else { return nil }
    ...
    private func relativeToNotes(_ relativePath: String) throws -> String {
        let absolutePath = layout.brainRoot.appendingPathComponent(relativePath).path
        let notesPrefix = layout.notes.path + "/"
