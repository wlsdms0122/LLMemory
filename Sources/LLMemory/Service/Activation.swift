...
public enum Activation {
    ...
    // MARK: - Public
    @discardableResult
    static func deriveWindows(_ db: Database, now: Int) throws -> DeriveResult {
        ...
        try Config.set(watermarkKey, value: lastId, txDB: db)
    ...
    static func markUsed(
        ...
        let cutoff = now - usedLookbackSec
        ...
        for id in ids {
            ...
            guard let hit else { throw UsedError.notSurfaced(id) }
    ...
    static func stats(_ db: Database) throws -> Stats {
    ...
    // MARK: - Private
    private static func openWindow(_ db: Database, ts: Int, label: String?) throws -> Int64 {
        let row: Row?
    ...
    private static func parsePayload(_ raw: String?) -> RetrievalPayload? {
        guard let raw,
