//
//  ReplayRetrievalQueriesOperation.swift
//  LLMemory
//
...
// caller's business.
struct ReplayRetrievalQueriesOperation: GRDBReadOperation {
    struct Divergence {
        // MARK: - Property
        let query: String
        let baseline: [String]
    ...
    // MARK: - Public
    func execute(_ db: Database) throws -> Outcome {
        let logged = try FetchLoggedRetrievalQueriesOperation(limit: limit).execute(db)
    ...
    private func replay(
        _ db: Database,
        _ query: FetchLoggedRetrievalQueriesOperation.LoggedQuery,
        tuning: RetrievalTuning
    ) throws -> [String] {
        switch query.replay {
        case let .search(tags, limit):
            return try SearchNotesFTSOperation(
