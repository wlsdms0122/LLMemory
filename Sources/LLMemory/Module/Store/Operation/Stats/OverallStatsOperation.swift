...
struct OverallStatsOperation: GRDBReadOperation {
    ...
    // MARK: - Public
    func execute(_ db: Database) throws -> OverallStats {
        ...
        let tree = try FetchTreeOperation().execute(db)
        ...
        return OverallStats(
            ...
            activation: try FetchActivationStatsOperation().execute(db)
        )
    }
