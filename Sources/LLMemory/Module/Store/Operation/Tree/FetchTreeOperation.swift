...
struct FetchTreeOperation: GRDBReadOperation {
    ...
    // MARK: - Public
    func execute(_ db: Database) throws -> [TreeRow] {
        let all = try String.fetchAll(db, sql: "SELECT id FROM notes")
        let ids = prefix.map { value in all.filter { id in NoteAddress.id(id, isWithin: value) } } ?? all
        let depth = prefix.map { value in NoteAddress.labels(of: value).count } ?? 0
        ...
        for id in ids {
            guard let branch = NoteAddress.branch(of: id, depth: depth + 1) ?? prefix else { continue }
