...
struct MarkNotesUsedOperation: GRDBOperation {
    struct UsedOutcome {
        // MARK: - Property
        let noteId: String
        let signal: String
    ...
    init(
        ...
        self.now = now
        self.lookbackSec = lookbackSec
    }

    // MARK: - Public
    func execute(_ db: Database) throws -> [UsedOutcome] {
        var outcomes: [UsedOutcome] = []
        let cutoff = now - lookbackSec
