...
enum Validation {
    ...
    struct PassResult {
        ...
        var rejectBreakdown: [String: Int] = [:]
    ...
    // MARK: - Public
    @discardableResult
    static func validatePendingTerms(
        _ db: Database,
        noteIds: [String]? = nil
    ) throws -> PassResult {
        var sql = """
        ...
        guard !rows.isEmpty else { return PassResult() }
    ...
    @discardableResult
    static func rejectStalePending(
        _ db: Database,
        maxAgeSec: Int = 86400
    ) throws -> Int {
        let now = Int(Date().timeIntervalSince1970)
        ...
        try db.execute(sql: """
            ...
            """, arguments: [RejectReason.roundtripFail.rawValue, now, now - maxAgeSec])
    ...
    private static func idfCheck(
        _ db: Database,
        tokens: [String],
        totalNotes: Int,
        dfCeiling: Double
    ) throws -> IDFCheck {
        let measureCommon = totalNotes >= idfMinCorpus
        var anyKnown = false
        var allCommon = true
