...
public enum Links {
    ...
    static func insertLineage(
        ...
        guard let (source, destination) = normalize(src: src, dst: dst, kind: kind) else { return }
    ...
    static func linkSiblings(_ db: Database, ids: [String], now: Int) throws {
            ...
            for right in sorted.dropFirst(index + 1) {
                ...
                try db.execute(sql: """
                    INSERT OR IGNORE INTO note_links
                      (src, dst, kind, weight, created_at, last_activated_at)
                    VALUES (?, ?, ?, ?, ?, ?)
                    """, arguments: [src, dst, kindSibling, 1.0, now, now])
            }
        }
    }
    ...
    static func redirectForMerge(_ db: Database, fromId: String, intoId: String) throws {
        let cap = 1.0
        ...
        for row in rows {
            ...
            guard let (source, destination) = normalize(
                src: newSrc,
                dst: newDst,
                kind: kind
            ) else {
            ...
            if undirectedKinds.contains(kind) {
                try db.execute(sql: """
    ...
    static func normalizeUndirected(_ db: Database, nodeId: String) throws {
        let kinds = Array(undirectedKinds)
    ...
    static func hasInbound(_ db: Database, noteId: String, limit: Int = 5) throws -> [String] {
        let directed = Array(deleteBlockingKinds.intersection(directedKinds))
        let symmetric = Array(deleteBlockingKinds.intersection(undirectedKinds))
    ...
    static func fanForSplit(
        _ db: Database,
        fromId: String
    ) throws -> (outEdges: [Edge], inEdges: [Edge]) {
        ...
        let inboundRows = try Row.fetchAll(db, sql: """
            ...
            """, arguments: [fromId])
        let outEdges: [Edge] = outboundRows.map { row in
            Edge(
                other: row["dst"],
                kind: row["kind"],
                weight: row["weight"],
        ...
        let inEdges: [Edge] = inboundRows.map { row in
            Edge(
                other: row["src"],
                kind: row["kind"],
                weight: row["weight"],
    ...
    static func insertRedistributed(
        _ db: Database,
        src: String,
        dst: String,
        kind: String,
    ...
    static func rebirth(_ queue: any DatabaseWriter, rankedIds: [(String, Double)], cap: Double = 1.0) throws -> Int {
        ...
        return try queue.write { db in
            ...
            let kindPlaceholders = Array(repeating: "?", count: learnedKinds.count)
                .joined(separator: ",")
            arguments.append(contentsOf: Array(learnedKinds) as [DatabaseValueConvertible?])
    ...
    static func decayAndPrune(
        _ db: Database,
        factor: Double? = nil,
        floor: Double? = nil
    ) throws -> (decayed: Int, pruned: Int) {
        let factor = factor ?? Genome.double("links.decay_factor")
        let floor = floor ?? Genome.double("links.prune_floor")
        let kindPlaceholders = Array(repeating: "?", count: learnedKinds.count)
            .joined(separator: ",")
        let kinds = Array(learnedKinds) as [DatabaseValueConvertible?]
