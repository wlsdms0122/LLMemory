...
enum DB {
    // MARK: - Property
    static let schemaVersion: Int = 6
...
"""#
    ...
    nonisolated(unsafe) private static var lockDepth: Int = 0
    
    private static let lockMutex = NSLock()
    private static let writeSection = NSRecursiveLock()
    
    nonisolated(unsafe) static var versionChecked: Bool = false
    nonisolated(unsafe) static var queue: DatabaseQueue?
    
    // MARK: - Initializer
    // MARK: - Public
    static func connect() throws -> DatabaseQueue {
        if let queue {
            return queue
        }
        ...
        if !FileManager.default.fileExists(atPath: dbURL.path) {
            throw DBError.notInitialized(Paths.brainRoot.path)
        }
        
        let queue = try DatabaseQueue(path: dbURL.path, configuration: makeConfig())
        try checkSchemaVersion(queue)
    ...
    static func initDB(postSchema: ((Database) throws -> Void)? = nil) throws {
        let dataDirectory = Paths.dataDirectory
        
        if !FileManager.default.fileExists(atPath: dataDirectory.path) {
            throw DBError.dataDirMissing(dataDirectory.path)
        }
        
        let queue = try DatabaseQueue(path: Paths.db.path, configuration: makeConfig())
        try queue.write { db in
            try db.execute(sql: schema)
            let mismatches = try checkSchemaShape(db)
            if !mismatches.isEmpty {
                throw DBError.schemaShapeMismatch(
                    brainRoot: Paths.brainRoot.path,
    ...
    static func expectedShape() throws -> [String: (columns: Set<String>, indexes: Set<String>)] {
        let memory = try DatabaseQueue()
        
        return try memory.write { db in
    ...
    static func normalizedSQL(_ sql: String) -> String {
        var stripped: [String] = []
    ...
    static func checkSchemaShape(_ db: Database) throws -> [String] {
        var messages: [String] = []
        let expected = try expectedShape()
        let memory = try DatabaseQueue()
        let expectedDDL: [String: String] = try memory.write { memoryDB in
            try memoryDB.execute(sql: "PRAGMA foreign_keys = OFF")
            try memoryDB.execute(sql: schema)
            
            return try storedDDL(memoryDB)
        }
        let actualDDL = try storedDDL(db)
        ...
        let actualTables = Set(try String.fetchAll(db, sql: """
            SELECT name FROM sqlite_master
            WHERE type='table' AND name NOT LIKE 'sqlite_%' AND name NOT LIKE 'notes_fts%'
            """))
    ...
    static func writeLock<T>(_ body: () throws -> T) throws -> T {
        writeSection.lock()
    ...
    @discardableResult
    static func write<T>(_ body: (Database) throws -> T) throws -> T {
        try writeLock {
            let queue = try connect()
    ...
    static func reset() {
        ...
        precondition(
            lockDepth == 0,
            "DB.reset during a held write lock — rebinding home mid-write is a programming error"
        )
    ...
    private static func checkSchemaVersion(_ queue: DatabaseQueue) throws {
        if versionChecked { return }
        
        let storedVersion: Int = try queue.read { db in
            try Int.fetchOne(db, sql: "PRAGMA user_version") ?? 0
        }
        
        if storedVersion != schemaVersion {
            throw DBError.schemaVersionMismatch(
                brainRoot: Paths.brainRoot.path,
                db: storedVersion,
                code: schemaVersion
            )
    ...
    private static func acquireLock() throws {
        lockMutex.lock()
        ...
        if lockDepth == 0 {
            let result = c_flock(lockFD, LOCK_EX)
