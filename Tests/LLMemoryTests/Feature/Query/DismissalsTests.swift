...
struct DismissalsTests {
    ...
    func untargetedCorpusFindingFailsLoud() {
        // When
        let untargeted = home.lintScanner.corpusIssue(
            code: "some-corpus-rule",
            severity: "warn",
            .init("two things collide")
        )
        let targeted = home.lintScanner.corpusIssue(
            code: "some-corpus-rule",
            severity: "warn",
        ...
        #expect(!home.lintScanner.dismissibleCodes.contains("lint-rule-untargeted"))
