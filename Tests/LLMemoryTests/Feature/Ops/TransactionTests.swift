...
@Suite("Transaction Tests", .serialized)
struct TransactionTests {
    ...
    func emptyOpsRejected() {
        // When
        let result = Transaction.apply(["ops": [], "rationale": "x"])
    ...
    func dryRunDoesNotPersist() {
        // When
        let result = Transaction.dryRun(["ops": [createOp(id: "tx-dry-1")], "rationale": "test"])
    ...
    func stagedSectionIsVisibleToTheNextOp() {
        ...
        // Then
        #expect(Transaction.dryRun(staged).status == "ok")
        #expect(Transaction.apply(staged).status == "ok")
    }
    ...
    func createThenPatchChainSimulated() {
        ...
        // When
        let good = Transaction.dryRun([
            "ops": [create, patchOp(id: "tx-cp-1", section: "## A", content: "- ok")],
            "rationale": "test"
        ])
        let bad = Transaction.dryRun([
