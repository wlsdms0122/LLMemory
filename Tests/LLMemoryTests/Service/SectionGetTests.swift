...
struct SectionGetTests {
    ...
    func getSectionsSlicesSubtree() throws {
        ...
        // When
        let (note, slices, _) = try home.readScope { scope in try home.container.notes.getSections(scope, id: "doc", sections: ["## A"], sessionId: nil) }
    ...
    func getSectionsUnknownPathFailsLoud() throws {
        ...
        #expect(throws: SectionError.self) {
            _ = try home.readScope { scope in try home.container.notes.getSections(scope, id: "doc2", sections: ["## Nope"], sessionId: nil) }
        }
    }
    ...
    func tocListsPathsAndWords() throws {
        ...
        // When
        let (_, entries, _) = try home.readScope { scope in try home.container.notes.toc(scope, id: "doc3", sessionId: nil) }
