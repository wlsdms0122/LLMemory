...
struct LinksTests {
    ...
    func newLinkCreated() throws {
        ...
        // Then
        #expect(try Links.strengthen(pairs: [("temp-a", "temp-b")]) == 1)
    }
    ...
    func sameSrcDstSkipped() throws {
        ...
        // Then
        #expect(try Links.strengthen(pairs: [("temp-a", "temp-a")]) == 0)
    }
    ...
    func rebirthTouchesOnlyLearnedKinds() throws {
        ...
        // When
        _ = try Links.rebirth(noteIds: ["temp-a", "temp-b"])
    ...
    func expandCutIsDeterministicOnWeightTies() throws {
        ...
        // When
        let expanded = try Links.expand(noteIds: ["exp-hub"], limit: 3).map(\.id)
    ...
    func siblingRanksBelowLearnedAssociationButStaysEligible() throws {
        ...
        // When
        let expanded = try Links.expand(noteIds: ["sib-hub"], limit: 3).map(\.id)
    ...
    private func endpoints(of pair: (String, String)?) -> [String]? {
