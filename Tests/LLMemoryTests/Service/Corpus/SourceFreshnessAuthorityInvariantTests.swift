...
struct SourceFreshnessAuthorityInvariantTests {
    ...
    func restoringTheOriginalContentRecoversFreshness() throws {
        ...
        // When
        let drifted = try queue.write { db in try SourceVerifier().verifyAll(db, home.brain) }
        ...
        let recovered = try queue.write { db in try SourceVerifier().verifyAll(db, home.brain) }
