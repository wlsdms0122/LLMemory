...
struct SourceVerifyInvariantTests {
    ...
    private static func writeSourcedNote(_ layout: BrainLayout, _ id: String, source: URL, body: String = "# body") throws -> URL {
    ...
    func partialSourceDeletionIsDetectedDespiteFreshMtime() throws {
        ...
        let noteDirectory = home.layout.notes
        ...
        _ = try queue.write { db in try SourceVerifier().verifyAll(GRDBScope(db, home.brain), home.brain, now: future) }
    ...
    func opaqueRefsAreNotDriftTracked() throws {
        ...
        _ = try queue.write { db in try SourceVerifier().verifyAll(GRDBScope(db, home.brain), home.brain, now: 9_999_999_999) }
    ...
    func rebaseSourceRebaselinesAndClears() throws {
        ...
        let plainPath = home.layout.notes.appendingPathComponent("src-plain.md")
    ...
    func splitInheritsObservationForInheritedDeclaration() throws {
        ...
        let notePath = try Self.writeSourcedNote(home.layout, "src-sp", source: file,
            body: "## A\nalpha\n## B\nbeta\n")
        let queue = try home.storage.connect()
    ...
    func mergeSourceChangeRebaselines() throws {
        ...
        let intoPath = try Self.writeSourcedNote(home.layout, "src-mi", source: first)
        let queue = try home.storage.connect()
    ...
    func splitRestatedSameSourceInheritsObservation() throws {
        ...
        let notePath = try Self.writeSourcedNote(home.layout, "src-rs", source: file,
            body: "## A\nalpha\n## B\nbeta\n")
        let queue = try home.storage.connect()
