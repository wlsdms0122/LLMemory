...
struct SourceGateInvariantTests {
    ...
    @discardableResult
    private static func writeNote(_ layout: BrainLayout, _ id: String, source: String) throws -> URL {
    ...
    func malformedOpsSourceIsRejectedByDryRunAndApply() throws {
        ...
        for bad in [[123], [NSNull()], [""], [["foo": "bar"]]] as [Any] {
            ...
            let dryRun = OperationsEngine.dryRun(home.storage, home.brain, ["ops": [op], "rationale": "test"])
    ...
    func validShapesStillApply() throws {
        ...
        let path = home.layout.notes.appendingPathComponent("gate-5.md")
        let (document, _) = try frontmatter.parse(try String(contentsOf: path, encoding: .utf8))
