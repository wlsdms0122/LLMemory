...
struct TemplateTests {
    ...
    // ops integration
    private func makeTemplate(locked: Bool = true) -> Transaction.Result {
        Transaction.apply(["ops": [[
    ...
    func documentWithTemplateScaffoldsFullFrame() throws {
        ...
        let created = Transaction.apply(["ops": [[
    ...
    func deleteDriftedDocumentSucceeds() throws {
        ...
        #expect(result.status == "ok")
        #expect(try !queue.read { db in try Notes.exists(db, nid: "doc-d") })
    }
    ...
    func queryTemplateReturnsFrameSchema() throws {
        ...
        let (note, frame) = try QueryFeature.template(home: home.path, id: "tpl-spec")
