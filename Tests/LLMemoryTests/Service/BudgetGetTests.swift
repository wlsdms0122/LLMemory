...
struct BudgetGetTests {
    ...
    func budgetCutsAtTopLevelBoundaryPrefixOnly() throws {
        ...
        let (_, _, cut) = try home.database().read { db in try NotesService.getBudget(GRDBScope(db), id: "bd", budget: 15) }
    ...
    func budgetIsAHardCapEvenForHeadAndFirstUnit() throws {
        ...
        let (_, _, cut) = try home.database().read { db in try NotesService.getBudget(GRDBScope(db), id: "bd2", budget: 1) }
    ...
    func budgetHugeFirstUnitAfterEmptyHeadIsLineCutAndNamed() throws {
        ...
        let (_, _, cut) = try home.database().read { db in try NotesService.getBudget(GRDBScope(db), id: "bd6", budget: 10) }
    ...
    func budgetLargeEnoughIsNotTruncated() throws {
        ...
        let (note, _, cut) = try home.database().read { db in try NotesService.getBudget(GRDBScope(db), id: "bd3", budget: 10_000) }
    ...
    func singletonWrapperDescendsToChildrenAsCutUnits() throws {
        ...
        let (_, _, cut) = try home.database().read { db in try NotesService.getBudget(GRDBScope(db), id: "bd5", budget: 16) }
        ...
        let (_, slices, _) = try home.database().read { db in
            try NotesService.getSections(GRDBScope(db), id: "bd5", sections: [cut.omitted[0].path])
        }
    ...
    func hugeWrapperDirectContentIsLineCutNotShippedWhole() throws {
        ...
        let (_, _, cut) = try home.database().read { db in try NotesService.getBudget(GRDBScope(db), id: "bd7", budget: 30) }
    ...
    func headingFreeBodyIsLineCutUnderBudget() throws {
        ...
        let (_, _, cut) = try home.database().read { db in try NotesService.getBudget(GRDBScope(db), id: "bd4", budget: 3) }
        
        #expect(cut.truncated)
        #expect(cut.truncatedWithin == "(preamble)")
        #expect(cut.shown == "three words here")
        
        let (note, _, full) = try home.database().read { db in try NotesService.getBudget(GRDBScope(db), id: "bd4", budget: 100) }
