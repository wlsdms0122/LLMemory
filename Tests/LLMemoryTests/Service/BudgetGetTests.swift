...
struct BudgetGetTests {
    ...
    func budgetCutsAtTopLevelBoundaryPrefixOnly() throws {
        ...
        let (_, _, cut) = try Retrieval.getBudget(home.database(), id: "bd", budget: 15)
    ...
    func budgetIsAHardCapEvenForHeadAndFirstUnit() throws {
        ...
        let (_, _, cut) = try Retrieval.getBudget(home.database(), id: "bd2", budget: 1)
    ...
    func budgetHugeFirstUnitAfterEmptyHeadIsLineCutAndNamed() throws {
        ...
        let (_, _, cut) = try Retrieval.getBudget(home.database(), id: "bd6", budget: 10)
    ...
    func budgetLargeEnoughIsNotTruncated() throws {
        ...
        let (note, _, cut) = try Retrieval.getBudget(home.database(), id: "bd3", budget: 10_000)
    ...
    func singletonWrapperDescendsToChildrenAsCutUnits() throws {
        ...
        let (_, _, cut) = try Retrieval.getBudget(home.database(), id: "bd5", budget: 16)
        ...
        let (_, slices, _) = try Retrieval.getSections(home.database(), 
    ...
    func hugeWrapperDirectContentIsLineCutNotShippedWhole() throws {
        ...
        let (_, _, cut) = try Retrieval.getBudget(home.database(), id: "bd7", budget: 30)
    ...
    func headingFreeBodyIsLineCutUnderBudget() throws {
        ...
        let (_, _, cut) = try Retrieval.getBudget(home.database(), id: "bd4", budget: 3)
        
        #expect(cut.truncated)
        #expect(cut.truncatedWithin == "(preamble)")
        #expect(cut.shown == "three words here")
        
        let (note, _, full) = try Retrieval.getBudget(home.database(), id: "bd4", budget: 100)
