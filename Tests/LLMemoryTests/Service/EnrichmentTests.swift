...
struct EnrichmentTests {
    ...
    func proposeLinkInsertsDormantAssocEdge() throws {
        ...
        let assocExpanded = try Links.expand(noteIds: ["enr-pl-a"], hops: 1, kind: Links.kindAssoc)
        
        #expect(!assocExpanded.contains { hit in hit.id == "enr-pl-b" })
        
        _ = try Links.strengthen(pairs: [("enr-pl-a", "enr-pl-b")], kind: Links.kindAssoc, step: 0.3)
        
        let after = try Links.expand(noteIds: ["enr-pl-a"], hops: 1, kind: Links.kindAssoc)
