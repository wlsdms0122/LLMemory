...
- **기본 지식(seed)** — 배포본이 싣고 오는 `locked: true` 노트(뇌 *안*의 지식).
...
기본 지식을 원하지 않는 brain 은 매 호출에 `--no-seed` 를 준다 (brain 이 기억하는 설정이 아니다).

기본 지식 노트는 frontmatter 에 **`seed: true`** 를 달고 온다. 이게 "이 자리는 배포본의 것"이라는
유일한 표식이고, 전용 디렉터리 없이도 남의 노트와 구별되는 이유다. 그래서 배포본이 새로 추가한 id 를
**이미 저작 노트가 차지하고 있으면 충돌**이다 — 그땐 *아무것도 심지 않고* 충돌한 id 를 나열하며 exit 1
한다. 그 노트를 다른 id 로 옮기거나, 정말 덮을 거면 `--force`.

**`update` 는 복사가 아니라 대조다.** 배포본이 싣고 오는 id 만 훑으면 *더 이상 안 싣게 된* id 는 영영
안 보인다 — 아무도 갱신하지 않고 아무도 못 찾는데 계속 배포본 것이라고 주장하는 노트가 남는다. 그런
id 는 **`cortex/.trash/` 로 옮기고 `retired` 로 보고**한다 (지우지 않는다 — 배포본은 자기 사본의
소유를 놓을 뿐, 사람이 남긴 것을 없앨 자리가 아니다). 계속 갖고 싶으면 `seed: true` 줄을 지워
자기 노트로 삼으면 된다.
...
<state-root>/
  ...
  cortex/
    a/b/c.md          — id `a.b.c` 의 노트. 경로는 id 의 함수이고, 그 아래에
                        ...
                        기본 지식(seed)도 여기 섞여 앉는다 — 전용 디렉터리가 없다.
                        그 id 들은 init/update 가 배포본으로 다시 쓴다 (`--no-seed` 로 생략).
