...
> **이 관행은 릴리스 전까지만 유효하다.** 릴리스가 나기 전에는 baseline 이 정본이고,
> 이전 모양의 brain 은 전부 이 스크립트로 한 번에 올라탄다 — 그래서 스키마 변경은 카탈로그를
> 늘리는 게 아니라 baseline 을 고친다. `notes.seed` 도 한때 `Migration2` 로 쌓였다가 baseline 으로
> 접혔다.
>
> **릴리스 이후에는 반대다.** baseline 은 이미 적용을 마친 brain 이 다시는 돌리지 않는 코드라,
> 거기를 고치면 *아직 존재하지 않는 brain* 의 스키마만 자란다 — 기존 brain 은 컬럼이 영영
> 안 생기고 형상 게이트(`ddl-mismatch`)에 걸려 모든 명령이 멈춘다. 그때부터는 migration 을
> 쌓는다.
>
> 쌓아도 형상 게이트와 충돌하지 않는다 (실측): 게이트는 *카탈로그 전체를 돌려 나온* DDL 과
> 실제 DB 의 DDL 을 대조하는데, 갓 만든 brain 도 이행된 brain 도 같은 카탈로그를 끝까지
> 돌므로 결과 DDL 이 같아진다 — baseline CREATE 끝에 컬럼이 이어붙은 모양으로 양쪽이 일치한다.
