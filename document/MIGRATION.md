...
> **이 관행은 배포 전까지만 유효하다.** baseline 을 고치면 *그 baseline 으로 만들어진* DB 는
> 스키마 형상 게이트에 걸려 모든 명령이 멈춘다 (`ddl-mismatch`). 지금은 그런 brain 이 개발 중의
> 임시본뿐이라 값이 싸지만, 배포된 순간부터는 실사용 brain 이 그 자리에 온다.
>
> **그리고 배포 뒤에는 `ALTER TABLE` migration 을 쌓는 것으로도 못 푼다** (실측):
> `SchemaShape` 가 sqlite_master 의 DDL *문자열*을 코드 스키마와 대조하는데, ALTER 로 붙인
> 컬럼은 원본 CREATE 문 끝에 이어붙어 baseline 의 문자열과 달라진다. 컬럼을 채워도
> `ddl-mismatch` 가 그대로 남는다. 배포 후 스키마를 넓히려면 이 대조를 문자열이 아니라
> *의미*(컬럼·타입·제약) 비교로 바꾸는 일이 먼저다 — 게이트를 약하게 만드는 변경이므로
> 별도 판단이 필요하다.
