...
> **이 관행은 이 재정의 한 번으로 끝난다.** 이후의 스키마 변경은 baseline 을 고치는 게 아니라
> **migration 을 쌓는다.** baseline 은 이미 적용을 마친 brain 이 다시는 돌리지 않는 코드라,
> 거기를 고치면 *아직 존재하지 않는 brain* 의 스키마만 자란다 — 기존 brain 은 컬럼이 영영
> 안 생기고 형상 게이트(`ddl-mismatch`)에 걸려 모든 명령이 멈춘다.
>
> migration 을 쌓으면 형상 게이트와 충돌하지 않는다 (실측): 게이트는 *카탈로그 전체를 돌려
> 나온* DDL 과 실제 DB 의 DDL 을 대조하는데, 갓 만든 brain 도 이행된 brain 도 같은 카탈로그를
> 끝까지 돌므로 결과 DDL 이 같아진다 — baseline CREATE 끝에 컬럼이 이어붙은 모양으로 양쪽이
> 일치한다. `notes.seed` 가 그 첫 사례다 (`Migration2`).
